...
class CheckEvent < ApplicationRecord
  ...
  VALID_EVENT_TYPES = %w[printed voided reprinted batch_downloaded alignment_test].freeze

  validates :event_type, inclusion: { in: VALID_EVENT_TYPES }
  validates :check_number, presence: true, unless: -> { event_type == "alignment_test" }
