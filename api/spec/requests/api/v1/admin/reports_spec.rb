...
RSpec.describe "Api::V1::Admin::Reports", type: :request do
  ...
  describe "locked QuickBooks payroll visibility" do
    ...
    it "combines linked snapshots with committed payroll and discloses excluded rows" do
      ...
      adjustment = HistoricalPaycheckAdjustment.create!(
        ...
        filing_year: historical_paycheck.pay_date.year,
        filing_quarter: ((historical_paycheck.pay_date.month - 1) / 3) + 1,
  ...
  describe "GET /api/v1/admin/reports/payroll_register_csv" do
    ...
    it "rejects a voided pay period from every register format even when its native key is supplied directly" do
      ...
      %w[payroll_register payroll_register_csv payroll_register_pdf payroll_register_xlsx].each do |endpoint|
        get "/api/v1/admin/reports/#{endpoint}", params: { pay_run_key: "native:#{voided_period.id}" }
