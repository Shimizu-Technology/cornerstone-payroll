...
Recurring-adjustment incident update as of `2026-09-03T09:26:38+10:00`: PR [#142](https://github.com/Shimizu-Technology/cornerstone-payroll/pull/142) merged the correction and safe pre-deploy process. Render was configured with the direct migration connection and `bundle exec rails db:safe_prepare`; the deployment completed, and a post-deploy operator session verified the affected June 4 payroll before and after recalculation. PR [#143](https://github.com/Shimizu-Technology/cornerstone-payroll/pull/143) resolved the dependency advisory exposed by the first merged-main run, and follow-up main run [33687384704](https://github.com/Shimizu-Technology/cornerstone-payroll/actions/runs/33687384704) passed. The production screenshots remain in the access-controlled session. No named independent Cornerstone reviewer is recorded in source control, so Cornerstone's accounting acceptance is still a separate business signoff. Keep direct migration URLs out of long-lived container configuration and command output; never restore blanket database-session termination. See the [redacted incident and implementation record](MARK_ACCOUNTANT_FEEDBACK_AND_RECURRING_BONUS_INVESTIGATION_2026-09-02.md).
