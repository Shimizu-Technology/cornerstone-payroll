...
export const payPeriodsApi = {
  ...
  reconcileTimeTrackingImport: (id: number, data: { import_id: number; reconciliation_note: string; mappings: Array<{ source_user_id: string; employee_id: number | null }> }): Promise<{ data: { results: { reconciled: unknown[]; errors: unknown[] }; import: TimeTrackingImportData } }> =>
    api.post<{ data: { results: { reconciled: unknown[]; errors: unknown[] }; import: TimeTrackingImportData } }>(`/admin/pay_periods/${id}/reconcile_time_tracking_import`, data),
};
